# Medical Disclaimer

**GLiPra** — a product of **Leonava** (a Texas company)
**Effective date:** [DATE]
**Last updated:** [DATE]

> **DRAFT — REQUIRES ATTORNEY REVIEW BEFORE PUBLICATION**

---

## 1. Not Medical Advice

**GLiPra is not a medical device, clinical service, or substitute for professional medical advice, diagnosis, or treatment.**

All content available through GLiPra — including but not limited to pharmacist-authored educational tips, nutrition guidance, protein floor calculations, injection-cycle phase descriptions, medication level estimates, readiness scores, weight progress charts, AI-generated meal analyses, symptom-escalation notices, and any AI-generated coaching responses — is provided **for informational and wellness-support purposes only**.

Nothing in the Service constitutes, and nothing should be understood as constituting, medical advice, a clinical recommendation, a diagnosis, or a treatment plan.

---

## 2. No Healthcare Provider Relationship

**Use of GLiPra does not create a physician-patient, pharmacist-patient, or any other healthcare provider relationship between you and Leonava, or between you and any individual who contributed to the development of the app.**

GLiPra is designed by a licensed pharmacist but:
- Is not your pharmacist.
- Does not dispense medications.
- Does not review your prescriptions.
- Does not establish a pharmacist-patient relationship.
- Does not provide individualized pharmaceutical care.

Any pharmacist-authored content in the app is general educational information, not personalized professional advice.

---

## 3. Medication Level Estimates

The medication level estimator is a mathematical tool based on published pharmacokinetic half-life data for GLP-1 receptor agonists. It provides **estimates only**.

**Actual serum drug concentrations vary significantly based on individual factors including:**
- Individual metabolic rate
- Body composition and body weight
- Kidney and liver function
- Injection technique and site
- Absorption variability
- Concurrent medications
- Timing of meals and other factors

The estimated values displayed in the app **do not reflect your actual serum drug level** and should never be used to make decisions about medication dosage, timing, or continuation. Only a licensed healthcare provider can order and interpret serum drug level testing.

---

## 4. Protein and Nutrition Estimates

Protein floor calculations, daily nutrition targets, meal analyses, and macro estimates are generated algorithmically based on information you provide. These estimates:

- Are approximations, not clinical nutritional assessments.
- Are based on population-level research and may not apply to your individual situation.
- Do not account for all medical conditions, food allergies, drug-nutrient interactions, or other individual factors.
- Should not be substituted for advice from a registered dietitian, physician, or other qualified nutrition professional.

---

## 5. Symptom-Escalation Notices

GLiPra may display a symptom-escalation notice when you log certain combinations of symptoms. These notices are generated automatically based on rule-based algorithms and are intended to prompt you to **contact your prescriber or healthcare provider**.

**Symptom-escalation notices:**
- Are not a diagnosis.
- Are not a determination that you are experiencing a medical emergency.
- Are not a substitute for evaluation by a qualified healthcare professional.
- May be triggered in situations that do not require urgent medical attention, and may not be triggered in all situations that do.

**If you believe you are experiencing a medical emergency, call 911 (or your local emergency services) immediately. Do not wait for or rely on any in-app alert.**

---

## 6. AI-Generated Content

Certain features of GLiPra use artificial intelligence to generate responses, nutritional estimates, or guidance. AI-generated content:

- Is based on patterns in data and may contain inaccuracies.
- Is not reviewed by a healthcare provider before being displayed to you.
- Is not a substitute for professional medical or nutritional advice.
- May not account for your complete medical history, current medications, or individual health circumstances.

Leonava does not warrant the accuracy, completeness, or suitability of AI-generated content for your individual situation.

---

## 7. Accuracy of Information

While we work to ensure that educational content in GLiPra is accurate and based on credible sources, we cannot guarantee that all information is complete, current, or free from error. Medical knowledge evolves, and information that was accurate at publication may become outdated.

**Always consult your prescribing physician, pharmacist, or other qualified healthcare provider before:**
- Making any changes to your medication, dose, or injection schedule.
- Beginning, modifying, or stopping a diet or nutrition plan.
- Acting on any symptom-escalation notice or health alert from the app.
- Making any other health or medical decision.

---

## 8. Personal Responsibility

You are solely responsible for your healthcare decisions. By using GLiPra, you acknowledge and agree that:

- You will not use the app as a substitute for professional medical advice.
- You will consult a qualified healthcare provider for any health concerns.
- You understand that the app provides general wellness support tools, not clinical care.
- Leonava is not responsible for any health outcome, clinical complication, or medical event arising from your use of or reliance on the Service.

---

## 9. Emergency Situations

**If you are experiencing a medical emergency, call 911 or your local emergency services immediately.**

Do not use GLiPra to seek emergency medical assistance. The app is not designed for, and should not be used in, emergency situations.

---

## 10. Limitation of Liability

TO THE MAXIMUM EXTENT PERMITTED BY APPLICABLE LAW, LEONAVA DISCLAIMS ALL LIABILITY FOR ANY HEALTH OUTCOME, CLINICAL COMPLICATION, INJURY, OR MEDICAL EVENT ARISING FROM OR RELATED TO YOUR USE OF OR RELIANCE ON ANY CONTENT OR FEATURE OF THE SERVICE. SEE THE TERMS OF SERVICE FOR THE COMPLETE LIMITATION OF LIABILITY.

---

*This Medical Disclaimer is incorporated into and forms part of the GLiPra Terms of Service. Defined terms used here have the meanings given in the Terms of Service.*

---

> **ATTORNEY REVIEW REQUIRED.** This is a first draft. Key areas: (1) Adequacy of half-life disclaimer language for medication level estimator; (2) Symptom-escalation notice liability exposure — verify language is protective; (3) FTC compliance for health claims; (4) FDA considerations — whether any feature could be construed as a software-as-a-medical-device (SaMD) and whether a 510(k) exemption applies; (5) Coordination with state consumer protection law.
