...
// Returns the key as-is â€” makes assertions easy
...
describe('buildReadinessCard', () => {
  ...
  it('passes score through unchanged', () => {
    const card = buildReadinessCard(makeResult(83), 'adjustment', null, mockT);
    expect(card.score).toBe(83);
  });

  it('passes score of 0 through unchanged', () => {
    const card = buildReadinessCard(makeResult(0), 'overdue', null, mockT);
  ...
  it('gives sentiment "negative" to a factor with negative delta', () => {
    ...
    const card = buildReadinessCard(result, 'adjustment', null, mockT);
  ...
  it('sorts negatives first (most negative first), then positives', () => {
    ...
    const card = buildReadinessCard(result, 'peak_suppression', null, mockT);
  ...
  it('tie-break: protein_pace beats other factors when injection_phase absent', () => {
    ...
    const card = buildReadinessCard(result, 'recovery_window', null, mockT);
