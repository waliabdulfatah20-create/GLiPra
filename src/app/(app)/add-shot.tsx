// NOTE: @react-native-community/datetimepicker requires native compilation.
// Date/time fields are display-only until the next EAS dev build (which commits
// the package) is installed on device. TODO: restore DateTimePicker after that build.
...
export default function AddShotScreen() {
  ...
  const [time] = React.useState<Date>(new Date());
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        {/* TIME TAKEN — display-only until next EAS build */}
        <Text style={styles.sectionLabel}>TIME TAKEN</Text>

        <View
          style={styles.fieldRow}
          accessibilityLabel={`Date: ${format(date, 'MMMM d, yyyy')}`}
        >
          <View style={styles.fieldLabelCol}>
            <Text style={styles.fieldLabel}>Date</Text>
            <Text style={styles.fieldValue}>{format(date, 'MMMM d, yyyy')}</Text>
          </View>
          <Text style={styles.fieldNote}>Now</Text>
        </View>

        <View
          style={styles.fieldRow}
          accessibilityLabel={`Time: ${format(time, 'h:mm a')}`}
        >
          <View style={styles.fieldLabelCol}>
